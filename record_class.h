...
// Grab a single block from the Dept.csv file and put it inside the DeptRecord structure of the Records Class
Records Grab_EmpDept_Record(fstream &deptin) {
    string line, word;
    //DeptRecord dept;
    Records dept;
    if (getline(deptin, line, '\n')) {
...
void PrintRecord(Records r){
	cout<<"Print ";
	cout<<r.emp_record.eid<<" "<<r.emp_record.ename<<" "<<r.emp_record.age<<" "<<r.emp_record.salary<<" ";
	cout<<r.dept_record.did<<" "<<r.dept_record.dname<<" "<<r.dept_record.budget<<" "<<r.dept_record.managerid<<r.no_values<<endl;
}